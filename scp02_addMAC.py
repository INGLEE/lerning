...
def dealFile(filepath):
	# print(filepath)
	pattern = '(.*?测试脚本_234G)'
	newfilepath=re.sub(pattern,filepath2+'/测试脚本_234G',filepath)
	pattern ='log'
			...
			if s1==('8482'):
				...
				if s=='84':
					if 'SW' in i:
						j=i.split(';')
						k=j[0]
						f2.write('\tFun_MAC2(0000000000000000,[621:630],[661:668]+'+k[:-16]+',[661:668])\n')
						f2.write(k[:-16]+'+[661:668]'+';'+j[1]+'\n')
					else:
						f2.write('\tFun_MAC2(0000000000000000,[621:630],[661:668]+'+log[count][:-17]+',[661:668])\n')
						f2.write(log[count][:-17]+'+[661:668] \n')	
				else:
					f2.write(log[count])	
			count +=1
	f2.close()
...
def eachFile(filepath):
    ...
    for i in cf:
       ...
       if os.path.isfile(child) : #如果是文件
       	if (os.path.splitext(child)[1]==".txt"):#判断是否是log
       		# print("是log")
